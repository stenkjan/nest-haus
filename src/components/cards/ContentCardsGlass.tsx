...
export default function ContentCardsGlass({ 
    ...
    return (
      <div className={containerClasses}>
        ...
        <div className="flex justify-center items-center py-8">
          <div className="animate-pulse bg-gray-800 rounded-3xl" style={{ width: 320, height: 480 }} />
        </div>
      </div>
    );
  }
  ...
  const getCardText = (card: CardData | StaticCardData, field: 'title' | 'subtitle' | 'description') => {
    ...
    <div className={containerClasses}>
      ...
      <div className={`relative ${isLightboxMode ? 'py-2' : 'py-8'}`}>
        ...
        <div className="overflow-x-clip">
          <div 
            ref={containerRef}
              className={`overflow-x-hidden ${maxWidth ? 'px-8' : 'px-4'} ${isStatic ? '' : 'cursor-grab active:cursor-grabbing'}`}
          ...
          >
            ...
            >
              {displayCards.map((card, index) => (
                <motion.div
                  key={card.id}
                    className={`flex-shrink-0 rounded-3xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 ${(isStatic && isClient && screenWidth >= 1024) || (isResponsive && isClient && screenWidth >= 1024) ? 'flex' : ''}`}
                                      style={{ 
                    width: cardWidth, 
                    height: isStatic ? (isClient && screenWidth >= 1600 ? 
                    ...
                    backgroundColor: '#121212',
                    boxShadow: 'inset 0 6px 12px rgba(255, 255, 255, 0.15), 0 8px 32px rgba(0, 0, 0, 0.3)'
                  }}
          ...
          <>
            {currentIndex > 0 && (
              <button
                ...
                className={`absolute left-0 top-1/2 transform -translate-y-1/2 ${maxWidth ? '-translate-x-4' : 'translate-x-2'} bg-gray-800 hover:bg-gray-700 rounded-full p-3 shadow-lg transition-all duration-200 hover:scale-110 z-10`}
              >
            ...
            {currentIndex < displayCards.length - Math.floor(cardsPerView) && (
              <button
                onClick={() => navigateCard(1)}
                className={`absolute right-0 top-1/2 transform -translate-y-1/2 ${maxWidth ? 'translate-x-4' : '-translate-x-2'} bg-gray-800 hover:bg-gray-700 rounded-full p-3 shadow-lg transition-all duration-200 hover:scale-110 z-10`}
              >
                <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
