...
export default function SummaryPanel({ onInfoClick, className = '' }: SummaryPanelProps) {
  ...
  const getItemPrice = (key: string, selection: Record<string, unknown>): number => {
    ...
    if (['gebaeudehuelle', 'innenverkleidung', 'fussboden'].includes(key)) {
      ...
      if (dynamicPrice && dynamicPrice.type === 'upgrade' && dynamicPrice.amount) {
    ...
    if (key === 'pvanlage') {
      return ((selection.quantity as number) || 1) * ((selection.price as number) || 0);
    }
    if (key === 'fenster') {
      return ((selection.squareMeters as number) || 1) * ((selection.price as number) || 0);
    }
  ...
  const isItemIncluded = (key: string, selection: Record<string, unknown>): boolean => {
          ...
          <div className="space-y-4">
            ...
            {Object.entries(configuration).map(([key, selection]) => {
              ...
              if (key === 'nest' || key === 'gebaeudehuelle' || key === 'innenverkleidung' || key === 'fussboden') {
                ...
                return (
                  <div key={key} className="flex justify-between items-start border-b border-gray-100 pb-3 gap-4">
                    ...
                    <div className="flex-1 text-right max-w-[50%] min-w-0">
                      {key === 'nest' ? (
                        <>
                          <div className="mb-1 text-black text-[clamp(12px,2.5vw,14px)]">Startpreis</div>
                          <div className="text-black text-[clamp(13px,3vw,15px)] font-medium">{PriceUtils.formatPrice(selection.price || 0)}</div>
                        </>
                      ) : (!isIncluded && itemPrice > 0) ? (
                        <>
                          <div className="mb-1 text-black text-[clamp(12px,2.5vw,14px)]">Aufpreis</div>
                          <div className="text-black text-[clamp(13px,3vw,15px)] font-medium">{PriceUtils.formatPrice(itemPrice)}</div>
                        </>
                      ) : (
                        <div className="text-gray-500 text-[clamp(12px,2.5vw,14px)]">inkludiert</div>
                      )}
                    </div>
                  </div>
                )
