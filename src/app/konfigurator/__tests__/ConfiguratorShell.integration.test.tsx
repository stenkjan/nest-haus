...
describe('ConfiguratorShell Integration', () => {
  beforeEach(() => {
    ...
    (PriceCalculator.getOptionDisplayPrice as jest.Mock).mockImplementation(
      (nestValue, selections, category, value) => {
        if (value === 'trapezblech' || value === 'kiefer' || value === 'parkett') {
    ...
    (PriceCalculator.getPriceCacheInfo as jest.Mock).mockReturnValue({ size: 0, keys: [] });
  ...
  it('should show upgrade prices for premium options', async () => {
  ...
  describe('Performance Optimization Tests', () => {
    it('should use memoized price calculations', () => {
      const { rerender } = render(<ConfiguratorShell rightPanelRef={mockRightPanelRef} />);

      // Initial render should call price calculator
      expect(PriceCalculator.getOptionDisplayPrice).toHaveBeenCalled();
      
      const initialCallCount = (PriceCalculator.getOptionDisplayPrice as jest.Mock).mock.calls.length;
      ...
      expect((PriceCalculator.getOptionDisplayPrice as jest.Mock).mock.calls.length)
    ...
    it('should demonstrate performance improvements through caching', async () => {
      ...
      console.log(`🔧 Configurator interaction time: ${totalTime.toFixed(2)}ms`);
    });
  });
