...
export default function MilestonesManager() {
  ...
  const handleAddMilestone = async () => {
    try {
      ...
      // Only consider M-numbered milestones, not Px or ZIEL
      const startDateForNew = new Date(newMilestone.startDate);

      // Get all existing M-numbered milestones sorted by date
      const mMilestones = allTasks
        .filter((t: ProjectTask) => /^M\d+$/.test(t.taskId))
        .sort(
          (a: ProjectTask, b: ProjectTask) =>
            new Date(a.startDate).getTime() - new Date(b.startDate).getTime()
        );

      // Find where this milestone should be inserted chronologically
      let milestoneNumber = 1;
      for (const milestone of mMilestones) {
        const milestoneDate = new Date(milestone.startDate);
        if (startDateForNew >= milestoneDate) {
          // This new milestone comes after this existing one
          ...
          // New milestone comes before this one, stop here
          break;
        }
      }
      ...
      const subsequentMilestones = mMilestones.filter((m: ProjectTask) => {
        const mDate = new Date(m.startDate);
        return mDate > startDateForNew;
      });

      if (subsequentMilestones.length > 0) {
        // Renumber all subsequent milestones
        const renumberPromises = subsequentMilestones.map(
          async (milestone: ProjectTask, index: number) => {
            const oldNumber = parseInt(
              milestone.taskId.match(/^M(\d+)$/)?.[1] || "0"
            );
            const newNumber = milestoneNumber + index + 1;

            // Only update if the number actually changed
            if (oldNumber !== newNumber) {
              const newTaskId = `M${newNumber}`;
              console.log(
                `Renumbering ${milestone.taskId} -> ${newTaskId} (${milestone.task})`
              );

              await fetch(`/api/admin/pmg/${milestone.id}`, {
                method: "PUT",
                headers: {
                  "Content-Type": "application/json",
                },
      ...
      // Reload all milestones from server to ensure IDs are properly synchronized
