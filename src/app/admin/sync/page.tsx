        ...
        {syncStatus ? (
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <span
                className={`w-3 h-3 rounded-full ${syncStatus.status === "ready" ? "bg-green-500" : "bg-red-500"}`}
              ></span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
              <div className="flex items-center gap-2">
                <span
                  className={`w-2 h-2 rounded-full ${syncStatus.configuration.googleDriveConfigured ? "bg-green-500" : "bg-red-500"}`}
                ></span>
                <span className="text-sm">Google Drive</span>
              </div>
              <div className="flex items-center gap-2">
                <span
                  className={`w-2 h-2 rounded-full ${syncStatus.configuration.blobConfigured ? "bg-green-500" : "bg-red-500"}`}
                ></span>
                <span className="text-sm">Vercel Blob</span>
              </div>
              <div className="flex items-center gap-2">
                <span
                  className={`w-2 h-2 rounded-full ${syncStatus.configuration.serviceAccountConfigured ? "bg-green-500" : "bg-red-500"}`}
                ></span>
              ...
              {lastResult.result && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 text-sm">
                  ...
                  <div>
                    ...
                    <span className="font-medium ml-1">
                      {lastResult.result.duration}ms
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600">Images.ts Updated:</span>
                    <span className="font-medium ml-1">
                      {lastResult.result.imagesUpdated ? "Yes" : "No"}
                    </span>
                  </div>
                </div>
              )}
        ...
        <div className="space-y-3">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Get step-by-step instructions to cleanly remove the Google Drive
              sync module
            </p>
          </div>
        </div>
