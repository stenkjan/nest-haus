...
useEffect(() => {
  ...
  if (lastSelectionCategory === 'innenverkleidung' || lastSelectionCategory === 'fussboden') {
...
}, [configuration.innenverkleidung, configuration.fussboden, configuration.geschossdecke, lastSelectionCategory]);
```
...
3. Verify "Dein Nest Deine Konfiguration" boxes hidden
