...
class ImageSwapVerifier {
  ...
  async verify() {
    ...
    try {
      ...
      console.log('📋 Target: Images 134-173 with "steirische-eiche" or "holz-natur"');
  ...
  async initializeGoogleAuth() {
    try {
      ...
      throw new Error(`Google Drive auth failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
          ...
          for (const file of response.data.files) {
            ...
            if (numberMatch) {
              ...
              if (number >= 134 && number <= 173) {
                const hasSteirischemEiche = nameLower.includes('steirische-eiche');
                ...
                if (hasSteirischemEiche || hasHolzNatur) {
                  ...
                  console.log(`🎯 Found: ${file.name} (${hasSteirischemEiche ? 'steirische-eiche' : 'holz-natur'})`);
                }
              }
            }
          }
    ...
    try {
      ...
      for (const blob of blobs) {
        ...
        if (numberMatch) {
          ...
          if (number >= 134 && number <= 173) {
            const hasSteirischemEiche = pathnameLower.includes('steirische-eiche');
          ...
          for (const assignment of assignments) {
            ...
            if (match) {
              ...
              if (numberMatch) {
                ...
                if (number >= 134 && number <= 173) {
                  const hasSteirischemEiche = valueLower.includes('steirische-eiche');
  ...
  createSwapPairs(driveImages) {
    ...
    for (const image of driveImages) {
      ...
      const targetName = image.hasSteirischemEiche 
        ? image.name.replace(/steirische-eiche/gi, 'holz-natur')
        : image.name.replace(/holz-natur/gi, 'steirische-eiche');

      // Find the matching pair
      const matchingImage = driveImages.find(img => 
        !processed.has(img.number) && 
        img.number !== image.number &&
        this.normalizeForComparison(img.name) === this.normalizeForComparison(targetName)
      );
      ...
      if (matchingImage) {
        pairs.push({
          ...
          image2: {
            ...
            type: matchingImage.hasSteirischemEiche ? 'steirische-eiche' : 'holz-natur'
          },
          wouldSwapTo: {
            image1NewName: matchingImage.name,
            image2NewName: image.name
          }
        });
        ...
        console.log(`🔗 Pair ${image.number} (${image.hasSteirischemEiche ? 'SE' : 'HN'}) ↔ ${matchingImage.number} (${matchingImage.hasSteirischemEiche ? 'SE' : 'HN'})`);
      }
    }
  ...
  validateConsistency(result) {
    ...
    const orphanedDrive = result.targetImages.drive.filter(img => !pairedNumbers.has(img.number));
    result.summary.orphanedImages = orphanedDrive.length;

    orphanedDrive.forEach(img => {
      result.issues.push(`Orphaned image in Drive: ${img.name} (no matching pair found)`);
    });

    // Check missing in blob
    const driveNumbers = new Set(result.targetImages.drive.map(img => img.number));
    const blobNumbers = new Set(result.targetImages.blob.map(img => img.number));
    const constantNumbers = new Set(result.targetImages.imagesConstants.map(img => img.number));

    driveNumbers.forEach(num => {
      if (!blobNumbers.has(num)) {
        result.summary.missingInBlob++;
        result.issues.push(`Image ${num} exists in Drive but missing in Vercel Blob`);
      }
      if (!constantNumbers.has(num)) {
        result.summary.missingInConstants++;
        result.issues.push(`Image ${num} exists in Drive but missing in images.ts constants`);
      }
    });
  }
  ...
  generateSummary(result) {
    ...
    console.log(`   • Total target images found: ${result.summary.totalTargetImages}`);
    ...
    console.log(`   • Missing in Constants: ${result.summary.missingInConstants}`);
    console.log(`   • Issues found: ${result.issues.length}`);

    if (result.swapPairs.length > 0) {
      console.log('\n🔄 SWAP PLAN:');
      result.swapPairs.forEach((pair, index) => {
        console.log(`   ${index + 1}. ${pair.image1.number} (${pair.image1.type}) ↔ ${pair.image2.number} (${pair.image2.type})`);
        console.log(`      → ${pair.image1.number} will become: ${pair.wouldSwapTo.image1NewName}`);
        console.log(`      → ${pair.image2.number} will become: ${pair.wouldSwapTo.image2NewName}`);
      });
    }
...
if (require.main === module) {
  ...
  verifier.verify()
    .then(result => {
      ...
      fs.writeFile('image-swap-verification-report.json', JSON.stringify(result, null, 2))
        .then(() => console.log('📄 Detailed report saved to: image-swap-verification-report.json'))
